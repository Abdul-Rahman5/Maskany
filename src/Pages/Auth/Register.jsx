...
export default function Register() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const secondApiUrl = `http://66.45.248.247:8000/auth/sms/?phone=${form.phoneNumber}`;
      const secondApiRes = await axios.get(secondApiUrl);
        console.log(secondApiRes);
      setLoading(false);
      const token = res.data.token;
      cookie.set("solom", token);
      localStorage.setItem("OtpPhone",secondApiRes.data.otp)
      localStorage.setItem("phoneNumber",form.phoneNumber)
      Navigate("/otpPhone", { replace: true });
    } catch (err) {
      setLoading(false);
